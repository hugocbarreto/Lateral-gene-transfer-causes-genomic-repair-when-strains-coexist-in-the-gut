...
for (population in list_of_populations_bam_I) { # Loop to blast the reads of each position against Invader and Resident genome
  ...
  list <- as.list(list.match(position_BLAST, population1))
  
  for (p in unlist(list)) {
    ...
    cl_Res <- predict(bl_Res,
     ...
     if (length(cl_Res$Perc.Ident) == 0  & length(cl_Inv$Perc.Ident) == 0) { # Adds 0 if there are no matching reads with the resident genome
      
      cl_Res <- add_row(cl_Res) %>%  
        mutate(SubjectID = "4_ResidentBacterialGenome.fa", Perc.Ident = 0) %>%
        dplyr::select(QueryID,SubjectID, Perc.Ident)
      
      cl_Inv <- add_row(cl_Inv) %>%  
        mutate(SubjectID = "NC_000913.2", Perc.Ident = 0) %>%
        dplyr::select(QueryID,SubjectID, Perc.Ident)
      
      Merge <- rbind(cl_Res, cl_Inv) %>%
        pivot_wider(id_cols = "QueryID",names_from = "SubjectID",values_from = "Perc.Ident") 
      Merge <- Merge %>%
    ...
    if (length(cl_Res$Perc.Ident) == 0  & length(cl_Inv$Perc.Ident) > 0) { # Adds 0 if there are no matching reads with the resident genome
      ...
      cl_Inv <- cl_Inv %>% mutate(SubjectID = "NC_000913.2") %>%
        dplyr::select(QueryID,SubjectID, Perc.Ident)
    ...
    if (length(cl_Res$Perc.Ident) > 0  & length(cl_Inv$Perc.Ident) == 0) { # Adds 0 if there are no matching reads with the invader genome
      
      cl_Res <- cl_Res %>% 
        mutate(SubjectID = "4_ResidentBacterialGenome.fa") %>%
        dplyr::select(QueryID,SubjectID, Perc.Ident)
      
      cl_Inv <- cl_Res %>% mutate(SubjectID = "NC_000913.2", Perc.Ident = 0) %>%
        dplyr::select(QueryID,SubjectID, Perc.Ident)
    ...
    if (length(cl_Res$Perc.Ident) > 0 & length(cl_Inv$Perc.Ident) > 0) { # Merges results from BLAST
      
      Merge <- rbind(dplyr::select(cl_Res,QueryID,SubjectID, Perc.Ident), 
                     dplyr::select(cl_Inv,QueryID,SubjectID, Perc.Ident)) %>%
        pivot_wider(id_cols = "QueryID",names_from = "SubjectID",values_from = "Perc.Ident")
...
## IMPORTANT: The final output of mutations obtained here still needs to be checked in breseq output of the sequenced population, as this script does not remove mutations that have misaligned reads (see below example 1 for the mutation in PROKKA_04530 → in position 4702633) and more than one variant per read (see below example 2 for the mutation in rnfC_2 ← in position 2540950)

#### Example 1 (PROKKA_04530 → in position 4702633), in red are highlithed the reads that show a misalignment (small letters and white background).

![](Examples/Example_1.jpeg)

#### Example 2 (rnfC_2 ← in position 2540950), in red are highlighted the reads that show the multiple variants in the same read.
